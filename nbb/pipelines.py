...
class NbbPipeline:
    ...
    # Database setup
    conn = sqlite3.connect('nbb.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `nbb`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM nbb WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO nbb (date, title, link, content)"
